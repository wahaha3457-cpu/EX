...
public class MarketTradesWsHandler extends AbstractMarketWsHandler {

  public MarketTradesWsHandler(MarketWsHub hub) {
    super(hub);
  }
  ...
  public void afterConnectionEstablished(WebSocketSession session) {
    ...
    int limit = intParam(session, "limit", 50, 1, 200);
    ...
    String channel = "trades:" + t.name().toLowerCase() + ":" + sym + ":" + limit;
    hub.join(
        channel,
        session,
        () ->
            hub.ensurePoller(
                ...
                () -> hub.broadcast(channel, hub.payload("trades", hub.trades(t, sym, limit)))));
  }
