...
import {
  ADMIN_CONFIG_MODULE_REGISTRY,
  type ConfigModuleKey,
  type ConfigModuleColumn,
} from '@/config/adminConfigModules'
import {
  adminConfirm,
  mockDelay,
  toastInfo,
...
const moduleKey = computed(() => {
  const k = route.meta.configModule as ConfigModuleKey | undefined
  return k && ADMIN_CONFIG_MODULE_REGISTRY[k] ? k : 'marketOverview'
})

const def = computed(() => ADMIN_CONFIG_MODULE_REGISTRY[moduleKey.value])
...
function statusKey(row: Record<string, unknown>): string | undefined {
  const v = row.status ?? row.publishStatus ?? row.orderStatus
  return v != null ? String(v) : undefined
}

const moduleHasStatus = computed(() => rawRows.value.some((r) => statusKey(r) != null))

/** 筛选「待处理」时命中的状态（含草稿、审核、定时、维护、停用等） */
...
function cellDisplay(col: ConfigModuleColumn, row: Record<string, unknown>): string {
  const v = row[col.prop]
  if (v == null) return '—'
  return String(v)
}

function tagFor(col: ConfigModuleColumn, row: Record<string, unknown>) {
...
const statToneClass = (tone: string | undefined) => {
  if (tone === 'warn') return 'adm-cm-stat--warn'
  if (tone === 'ok') return 'adm-cm-stat--ok'
  if (tone === 'muted') return 'adm-cm-stat--muted'
  return ''
}
...
<template>
  <div class="adm-cm" v-loading="loading">
    <header class="adm-cm__hero">
      <div class="adm-cm__titles">
        <h1 class="adm-cm__h1">{{ def.title }}</h1>
        <p class="adm-cm__sub">{{ def.subtitle }}</p>
      </div>
      <span v-if="def.hint" class="adm-cm__hint">{{ def.hint }}</span>
    </header>

    <el-row v-if="def.stats?.length" :gutter="12" class="adm-cm__stats">
      <el-col v-for="(s, i) in def.stats" :key="i" :xs="24" :sm="12" :md="6" :lg="6">
        <div class="adm-cm-stat" :class="statToneClass(s.tone)">
          <p class="adm-cm-stat__lbl">{{ s.label }}</p>
          <p class="adm-cm-stat__val">{{ s.value }}</p>
        </div>
      </el-col>
    </el-row>
    ...
    >
      ...
      >
        <template #default="{ row }">
          ...
          <span v-else class="adm-cm__cell" :class="{ 'adm-cm__cell--mono': col.mono }">{{ cellDisplay(col, row) }}</span>
        </template>
      </el-table-column>
        ...
        <template #default="{ row }">
          <AdmActionGroup>
            ...
            <el-dropdown
              ...
              "
            >
              <el-button type="primary" link size="small" class="adm-cm__more">
                更多
      ...
      <template v-if="currentRow">
        <el-descriptions :column="1" border size="small">
          ...
          >
            ...
            <span v-else :class="{ 'adm-cm__cell--mono': col.mono }">{{ cellDisplay(col, currentRow) }}</span>
          </el-descriptions-item>
        </el-descriptions>
        <p class="adm-cm__drawer-hint">
          {{
            def.drawerNote ??
            '生产环境可挂载配置版本、灰度范围、操作人与审计流水。'
          }}
        </p>
      </template>
      <template #footer>
...
.adm-cm-stat--warn .adm-cm-stat__val {
  color: var(--el-color-warning);
}

.adm-cm-stat--ok .adm-cm-stat__val {
  color: var(--el-color-success);
}

.adm-cm-stat--muted .adm-cm-stat__val {
  color: var(--el-text-color-secondary);
}
