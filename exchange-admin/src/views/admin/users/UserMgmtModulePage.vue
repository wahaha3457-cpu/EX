...
const PENDING_KEYS = new Set(['PENDING', 'REVIEW', 'OPEN', 'WAIT_USER'])
  ...
  <div class="adm-um" v-loading="loading">
    ...
    <AdmFilterBar>
      <el-form-item :label="def.filterKeywordLabel ?? '关键词'">
        <el-input
          v-model="keyword"
          :placeholder="def.keywordPlaceholder ?? 'UID / 邮箱 / 手机号'"
          clearable
          style="width: 240px"
          :prefix-icon="Search"
    ...
    >
      ...
      >
        <template #default="{ row }">
          ...
          <span v-else class="adm-um__cell" :class="{ 'adm-um__cell--mono': col.mono }">{{ cellDisplay(col, row) }}</span>
        </template>
      </el-table-column>
      ...
      <template v-if="currentRow">
        <el-descriptions :column="1" border size="small">
          ...
          >
            ...
            <span v-else :class="{ 'adm-um__cell--mono': col.mono }">{{ cellDisplay(col, currentRow) }}</span>
          </el-descriptions-item>
        </el-descriptions>
        <p class="adm-um__drawer-hint">
          {{
            def.drawerNote ??
            '生产环境可在此挂载审核轨迹、附件与只读风控标签。'
          }}
        </p>
      </template>
      <template #footer>
