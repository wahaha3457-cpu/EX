...
async function handleExport() {
  ...
  if (rows.length === 0) {
    toastWarning(t('admin.placeholder.exportEmpty'))
    return
  }
  await withLoading(async () => {
    await mockDelay(100)
    const spec: CsvColumnPick<Row>[] = [
      { header: 'ID', key: 'id' },
      { header: t('admin.placeholder.colName'), key: 'name' },
      {
        header: t('admin.placeholder.colStatus'),
        key: 'status',
        format: (v: unknown) =>
          v === 'PENDING'
            ? t('admin.placeholder.statusPending')
            : t('admin.placeholder.statusActive'),
      },
    ]
    const csv = buildCsvFromKeyedRows(rows, spec)
    const base = sanitizeFilenameBase(String(t('admin.placeholder.tableTitle')))
    triggerUtf8BomCsvDownload(csv, `${base}_${timestampForFilename()}.csv`)
    toastSuccess(t('admin.placeholder.exportOk', { count: rows.length }))
  })
}
